...
$("#source-submit-push").onclick = async () => { const message = prompt("源码提交说明", "content: update from Firefly studio"); if (!message) return; managementOutput("正在提交源码..."); const commit = await projectAction("/api/git/commit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message }) }); if (!commit) return managementOutput("源码提交失败，请查看右下角提示。"); const push = await projectAction("/api/git/push", { method: "POST" }); if (push) { managementOutput(`${commit.output || "源码提交完成"}\n${push.output || "源码推送完成"}`); toast("源码已提交并推送"); } };
...
$("#artifact-submit-push").onclick = async () => { const message = prompt("构建产物提交说明", "deploy: update dist"); if (!message) return; managementOutput("正在提交构建产物..."); const commit = await projectAction("/api/git/artifact-commit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message }) }); if (!commit) return managementOutput("构建产物提交失败，请查看右下角提示。"); const push = await projectAction("/api/git/artifact-push", { method: "POST" }); if (push) { managementOutput(`${commit.output || "构建产物提交完成"}\n${push.output || "构建产物推送完成"}`); toast("构建产物已提交并推送"); } };
