...
const render = () => {
  ...
  $("#content-list").innerHTML = visible.length ? `<div class="content-table-head"><span>标题</span><span>状态</span><span>更新时间</span><span>类型</span><span>操作</span></div>${rows}` : `<div class="empty"><strong>${query ? "没有匹配的内容" : "这里还没有内容"}</strong><span>${query ? "换一个关键词，或清空搜索条件。" : "创建一篇文章或动态，内容会直接保存到 Firefly 项目。"}</span>${query ? "" : '<button class="workflow-button" data-action="new">创建第一条内容</button>'}</div>`;
