...
const render = () => {
  ...
  const rows = visible.map((item) => `<article class="content-row" data-id="${item.id}" tabindex="0"><div class="row-primary"><div class="row-title">${escapeHtml(item.title)}</div><div class="row-description">${escapeHtml(item.description || item.content.replace(/[#*`]/g, "").slice(0, 120))}</div></div><div class="row-status"><span class="badge ${item.status}">${item.status === "published" ? "已发布" : "草稿"}</span></div><time class="row-date">${formatDate(item.updatedAt)}</time><span class="row-type">${item.type === "article" ? "文章" : "动态"}</span><div class="row-actions"><button data-action="preview" data-id="${item.id}" data-tooltip="打开只读 Markdown 预览页">预览</button><button data-action="toggle" data-id="${item.id}" data-tooltip="${item.status === "published" ? "撤回为草稿，不会执行 Git、构建或部署" : "发布到 Firefly 内容目录，不会执行 Git、构建或部署"}">${item.status === "published" ? "撤回" : "发布"}</button><button data-action="delete" data-id="${item.id}" data-tooltip="永久删除源文件或本地草稿">删除</button></div></article>`).join("");
  $("#content-list").innerHTML = visible.length ? `<div class="content-table-head"><span>标题</span><span>状态</span><span>更新时间</span><span>类型</span><span class="actions-heading">操作<button class="help-button table-help-button" data-action="actions-help" aria-label="查看内容操作说明" title="内容操作说明"><i class="ph ph-question-mark" aria-hidden="true"></i></button></span></div>${rows}` : `<div class="empty"><strong>${query ? "没有匹配的内容" : "这里还没有内容"}</strong><span>${query ? "换一个关键词，或清空搜索条件。" : "创建一篇文章或动态，内容会直接保存到 Firefly 项目。"}</span>${query ? "" : '<button class="workflow-button" data-action="new">创建第一条内容</button>'}</div>`;
...
const updateTypeFields = () => { const dynamic = document.querySelector('input[name="type"]:checked').value === "dynamic"; document.querySelectorAll(".article-only").forEach((element) => { element.classList.toggle("hidden", dynamic); element.hidden = dynamic; }); document.querySelectorAll(".dynamic-only").forEach((element) => { element.classList.toggle("hidden", !dynamic); element.hidden = !dynamic; }); $("#title").required = !dynamic; $("#title-hint").textContent = dynamic ? "动态标题可选，用于后台识别" : "文章标题"; $("#title").placeholder = dynamic ? "用于后台识别，可不填" : "给这次创作一个名字"; };
...
$("#content-list").onclick = async (event) => {
  ...
  if (button?.dataset.action === "actions-help") { $("#content-actions-help-modal").classList.remove("hidden"); return; }
...
[["#editor-modal", () => $("#editor-modal").classList.add("hidden")], ["#project-help-modal", () => $("#project-help-modal").classList.add("hidden")], ["#remote-modal", () => $("#remote-modal").classList.add("hidden")], ["#code-management-modal", () => $("#code-management-modal").classList.add("hidden")], ["#content-actions-help-modal", () => $("#content-actions-help-modal").classList.add("hidden")], ["#action-dialog", () => $("#action-dialog").close("cancel")]].forEach(([selector, close]) => { const element = $(selector); element.onclick = null; bindBackdropClick(element, close); });
