...
const openEditor = (item = null) => { state.editingId = item?.id || null; state.editingSourcePath = item?.sourcePath || null; $("#editor-title").textContent = item ? "编辑内容" : "新建内容"; $("#title").value = item?.title || ""; $("#description").value = item?.description || ""; $("#category").value = item?.category || ""; $("#tags").value = item?.tags?.join(", ") || ""; $("#content").value = item?.content || ""; $("#markdown-preview").classList.add("hidden"); $("#content").classList.remove("hidden"); $("#preview-toggle").textContent = "预览"; document.querySelector(`input[name="type"][value="${item?.type || "article"}"]`).checked = true; updateTypeFields(); $("#editor-modal").classList.remove("hidden"); $("#title").focus(); };
const updateTypeFields = () => { const dynamic = document.querySelector('input[name="type"]:checked').value === "dynamic"; document.querySelectorAll(".article-only").forEach((element) => element.classList.toggle("hidden", dynamic)); $("#title").required = !dynamic; $("#title-hint").textContent = dynamic ? "动态标题可选，用于后台识别" : "文章标题"; };
const save = async (status) => { const payload = { type: document.querySelector('input[name="type"]:checked').value, title: $("#title").value, description: $("#description").value, category: $("#category").value, tags: $("#tags").value.split(",").map((tag) => tag.trim()).filter(Boolean), content: $("#content").value, status }; const url = state.editingId ? `/api/content/${state.editingId}` : "/api/content"; const response = await fetch(url, { method: state.editingId ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); const result = await response.json(); if (!response.ok) return toast(result.error); $("#editor-modal").classList.add("hidden"); toast(status === "published" ? "内容已发布" : "草稿已保存"); await refresh(); };
$("#new-button").onclick = () => openEditor(); $("#select-project").onclick = async () => { const button = $("#select-project"); button.disabled = true; button.textContent = "等待选择..."; try { const response = await fetch("/api/project/select", { method: "POST" }); const result = await response.json(); if (!response.ok) toast(result.error); else if (!result.cancelled) { toast("Firefly 项目连接成功"); await refresh(); } } catch { toast("无法打开文件夹选择器"); } finally { button.disabled = false; await refreshProject(); } }; $("#close-editor").onclick = () => $("#editor-modal").classList.add("hidden"); $("#editor-modal").onclick = (event) => { if (event.target === event.currentTarget) $("#editor-modal").classList.add("hidden"); }; $("#save-draft").onclick = () => save("draft"); $("#editor-form").onsubmit = (event) => { event.preventDefault(); save("published"); }; $("#preview-toggle").onclick = () => { $("#markdown-preview").innerHTML = renderMarkdown($("#content").value, state.editingSourcePath); $("#markdown-preview").classList.toggle("hidden"); $("#content").classList.toggle("hidden"); $("#preview-toggle").textContent = $("#content").classList.contains("hidden") ? "返回编辑" : "预览"; };
